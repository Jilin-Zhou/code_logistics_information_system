...
@Table(name = "t_out")  // JPA 注解，指定实体对应的数据库表名
public class Out extends BaseEntity implements Serializable {
    @Column(name = "dueDate")  // JPA 注解，将属性映射到数据库表的对应列
    private String dueDate;     // 要求的送货日期

    @OneToMany(mappedBy = "out", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    @JsonIgnore  // Jackson 注解，指定序列化时忽略该属性
    private Set<OutDetail> outDetailSet = new HashSet<>();  // 关联的采购订单详情集合
    ...
    public List<OutDetail> getOutDetails() {
        return new ArrayList<>(outDetailSet);
    }
}